...
class _SearchTapState extends State<SearchTap> {
  ...
  CollectionReference<UserProfileModel> _getUserDetailsCollection(String userId) {
  ...
  void _loadAllUsers() async {
    ...
    try {
      ...
      for (var userDoc in usersSnapshot.docs) {
        ...
        final userDetailsSnapshot = await _getUserDetailsCollection(userDoc.id).get();
        print('Found ${userDetailsSnapshot.docs.length} detail entries for user ${userDoc.id}');
                ...
                : _searchResults.isEmpty
                    ? Center(
                        child: _hasSearched
                            ...
                            : const Text('Search for users by name or skills'),
                      )
                    : ListView.builder(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  if (user.offeredSkills?.isNotEmpty == true)
                                    Wrap(
                                      spacing: 4,
                                      children: user.offeredSkills!.map((skill) {
                                        return Chip(
                                          label: Text(skill),
