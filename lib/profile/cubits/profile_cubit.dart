...
class ProfileCubit extends Cubit<ProfileState> {
  ...
  ProfileCubit({ProfileSetupState? initialData}) : super(const ProfileState(isLoading: true)) {
    if (initialData != null) {
      ...
      emit(ProfileState(
        ...
        bio: initialData.bio,
        profileImage: initialData.profileImage, // Use the File object directly
        offeredSkills: List.from(initialData.selectedOfferedSkills), // Create copies
        desiredSkills: List.from(initialData.selectedDesiredSkills), // Create copies
        skillCategories: const [], // Categories aren't part of setup state, load separately if needed
  ...
  Future<void> loadProfile() async {
    ...
    if (!state.isLoading) {
        print("ProfileCubit: loadProfile called but not in loading state, skipping.");
        return;
    }
    ...
    try {
      ...
      // Simulated data (used if no initialData was provided)
       ...
       final List<SkillCategory> skillCategories = [
         SkillCategory(name: 'Default Category', skills: ['Placeholder Skill 1', 'Placeholder Skill 2'])
