...
class ProfileSetupState extends Equatable {
  ...
  const ProfileSetupState({ // Use const constructor
  ...
  ProfileSetupState copyWith({
    ...
    bool clearProfileImage = false, // Add flag to explicitly clear image if needed
    String? fullName,
    String? bio,
    bool? isLoading,
    String? errorMessage,
    ...
    return ProfileSetupState(
      ...
      filteredOfferedSkills: filteredOfferedSkills ?? this.filteredOfferedSkills,
      filteredDesiredSkills: filteredDesiredSkills ?? this.filteredDesiredSkills,
      selectedOfferedSkills: selectedOfferedSkills ?? this.selectedOfferedSkills,
      selectedDesiredSkills: selectedDesiredSkills ?? this.selectedDesiredSkills,
  ...
  List<Object?> get props => [
        ...
        profileImage?.path, // Compare file path for simplicity, or use file content hash if needed
        fullName,
        bio,
        isLoading,
        errorMessage,
...
class ProfileSetupCubit extends Cubit<ProfileSetupState> {
  ...
  void filterOfferedSkills(String query) {
    ...
    if (query.isEmpty) {
      ...
      emit(state.copyWith(filteredOfferedSkills: List.from(state.skillOptions)));
    } else {
      final filtered = state.skillOptions
  ...
  void filterDesiredSkills(String query) {
     final lowerCaseQuery = query.toLowerCase();
    if (query.isEmpty) {
      emit(state.copyWith(filteredDesiredSkills: List.from(state.skillOptions)));
    } else {
      final filtered = state.skillOptions
          .where((skill) => skill.toLowerCase().contains(lowerCaseQuery))
          .toList();
  ...
  void setProfileImage(File image) {
    ...
    emit(state.copyWith(profileImage: image, errorMessage: '')); // Clear error on successful update
  }
  ...
  void filterByCategory(String categoryName) {
    if (categoryName == 'All') { // Handle an "All" category case
       emit(state.copyWith(
    ...
    final category = state.skillCategories.firstWhere(
      ...
      orElse: () => SkillCategory(name: '', skills: []), // Return empty if not found
    );
    ...
    if (category.skills.isNotEmpty) {
      ...
      // Or emit an empty list if that's the desired behavior?
       ...
       debugPrint("Category '$categoryName' not found or has no skills. Resetting filters.");
    }
  }
  ...
  Future<void> saveProfile() async {
      ...
      try {
          ...
          if (state.selectedOfferedSkills.isEmpty && state.selectedDesiredSkills.isEmpty) {
               throw Exception("Please select at least one skill you offer or want to learn.");
          }
          ...
          if (state.profileImage != null) {
              ...
              imageUrl = "https://example.com/path/to/uploaded/image.jpg"; // Placeholder URL
          }

          // 4. Send profile data (including image URL) to your backend API
          debugPrint("Simulating saving profile data: $profileData, Image URL: $imageUrl");
          // await yourApiService.saveUserProfile(profileData, imageUrl);
          await Future.delayed(const Duration(seconds: 1)); // Simulate network delay

          // 5. Emit success state (or navigate away)
          emit(state.copyWith(isLoading: false, errorMessage: '')); // Clear loading and error
